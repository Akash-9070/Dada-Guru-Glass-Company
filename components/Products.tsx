...
export default function Products() {
  ...
  return (
    <section className="py-20 bg-[#F5F5F5]">
      <div className="bg-[#F0E6D9] rounded-3xl p-8 md:p-12">
        <motion.h2
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[200px]">
          {products.map((product) => (
            <motion.div
              ...
              className={`relative overflow-hidden rounded-lg cursor-pointer ${product.span}`}
      ...
      <AnimatePresence>
        {selectedProduct && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-[#3E362E] bg-opacity-90 flex items-center justify-center z-50 p-4"
          ...
          >
            ...
            >
              ...
              <div className="p-4 bg-[#F5F5F5]">
                <h3 className="text-2xl font-semibold text-[#3E362E]">{selectedProduct.name}</h3>
              </div>
            </motion.div>
          </motion.div>
        )}
